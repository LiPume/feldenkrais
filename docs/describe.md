...
25. 老师账号现在也拥有反馈权限
    `/feedback`、`/feedback/new`、旧反馈导入和相关 Server Action 都已放开给老师与学生共用
26. 登录 / 注册已增加姓名和学号字段
    学生注册时会写入 `full_name` 和 `student_id`，老师端学生列表也会显示这些信息
27. 老师端学生统计已改成“所有已注册学生”
    现在会把 0 次反馈的学生也显示出来，老师可直接看出当前筛选范围内谁还没填写
28. 学生认证已切换为“学号 + 密码”
    老师继续使用邮箱登录；学生账号在 Supabase 内部仍有邮箱，但前台不再要求学生输入
29. 内部学生邮箱已从界面隐藏
...
12. 老师账号可以像学生一样提交和查看自己的反馈，只是额外多了老师统计页权限
13. 学生账号现在支持记录姓名和学号，老师端可直接看到姓名、学号和填写次数
