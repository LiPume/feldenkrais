...
import { FRONT_REGIONS } from '@/lib/constants/body-regions';
...
const REGION_CODES = FRONT_REGIONS.map((r) => r.code) as BodyRegionCode[];
...
function handleRegionKey(
  ...
  if (next !== null) {
    event.preventDefault();
    document.getElementById(`bodymap-front-${REGION_CODES[next]}`)?.focus();
  }
}

function FrontSkinBase() {
  return (
...
function FrontSkeleton() {
  return (
    <g aria-hidden="true" className="body-map-skeleton">
      ...
      <path className="body-map-bone" d="M145 82 Q160 96 175 82" />
      ...
      <path className="body-map-bone" d="M141 132 Q160 146 179 132" />
      <path className="body-map-bone" d="M126 151 Q160 174 194 151" />
      <path className="body-map-bone" d="M119 176 Q160 204 201 176" />
      <path className="body-map-bone" d="M120 204 Q160 232 200 204" />
      <path className="body-map-bone" d="M143 128 L108 151" />
      <path className="body-map-bone" d="M177 128 L212 151" />
      <path className="body-map-bone" d="M104 160 C92 190 86 222 82 251 C79 275 71 297 63 320" />
      <path className="body-map-bone" d="M216 160 C228 190 234 222 238 251 C241 275 249 297 257 320" />
...
export default function BodyMapFront({ selectedCodes, onToggle, layer }: Props) {
  ...
  return (
    <svg
      ...
      aria-label={`正面人体${layer === 'skeleton' ? '骨骼' : '皮肤'}图`}
      role="group"
    >
      <FrontSkinBase />
      {layer === 'skeleton' && <FrontSkeleton />}
